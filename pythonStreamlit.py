...
def process_video(uploaded_video):
    ...
    while(True):
        ...
        if len(centers) > 0:
            ...
            val, counts = np.unique(list(beesID.values()), return_counts=True)
            if 1 in val:
                beesIn =  counts[np.where(val==1)][0]
            allTracks = np.sum(counts)
            if 3 in val:
                pollenIn = counts[np.where(val==3)][0]
            if 2 in val:
                beesOut = counts[np.where(val==2)][0]
                
        if frame_number%100 == 0:
            records.append([timestamp, beesIn,pollenIn,beesOut])
        ...
        # Add dashboard texts
        cv2.putText(currentFrame, f"Total Count: {allTracks}", (10, 40),  cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 255), 2)
        cv2.putText(currentFrame, f"Bees In: {beesIn}", (10, 70),  cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 255), 2)
        cv2.putText(currentFrame, f"Pollen In: {pollenIn}", (10, 100),  cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 255), 2)
        cv2.putText(currentFrame, f"Bees Out: {beesOut}", (10, 130),   cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 255), 2)
